export const messages = {
  'zh-CN': {
    ...
    common: {
      ...
      download: '下载',
      copyLink: '复制链接',
      copyUnavailable: '本地转换结果没有可长期复用的链接',
      copySuccess: '下载链接已复制',
      empty: '暂无数据',
      processing: '处理中',
      completed: '已完成',
    ...
    home: {
      ...
      title: '把 HEIC 批量拖进来，直接在一个工作台里完成转换。',
      description:
        '首页现在只保留上传、队列、结果和参数控制。把文件放进来后，可以按缩略图查看状态、进度和结果动作，不再需要在多个介绍卡片之间来回跳转。',
      primaryAction: '开始转换',
      secondaryAction: '查看帮助',
      privacy: '浏览器端模式不会上传文件，服务器端模式更适合大批量任务。',
      statsTitle: '实时指标',
      dashboardTitle: '批量工作状态',
      queueTitle: '队列与结果',
      queueSubtitle: '所有文件会在同一网格中展示，完成后会直接切换为转化结果预览，并支持单张下载。',
      emptyWorkspace: '先把 HEIC / HEIF 文件拖进来，工作台会立刻生成缩略图和状态卡片。',
      uploadTitle: '投放文件',
    ...
    upload: {
      ...
      hint: '支持最多 80 个文件，单文件 30 MB，单批次总量 500 MB。',
      paste: '已启用粘贴图片支持',
      folder: '支持文件夹上传',
      preparing: '准备文件中',
      ready: '已就绪',
      queueTitle: '导入进度',
      queueHint: '文件会先完成校验并加入工作台队列，再进入上传或转换流程。',
      button: '选择文件',
      errorTitle: '文件检查未通过'
    },
    settings: {
      modeTitle: '处理路径',
      modeHint: '根据文件规模选择本地优先、云端加速或自动调度。',
      formatTitle: '输出格式',
      qualityTitle: '画质',
      qualityDescription: '推荐 82%，优先控制体积，默认尽量不超过原图 2 倍。',
      advanced: '高级选项',
  ...
  'en-US': {
    ...
    common: {
      ...
      download: 'Download',
      copyLink: 'Copy link',
      copyUnavailable: 'Local conversions do not have a persistent shareable link',
    ...
    home: {
      eyebrow: 'Phase 2 / Studio Refresh',
      title: 'Drop in a HEIC batch and finish the whole workflow from one workbench.',
      description:
        'The home screen now keeps only upload, queue, results, and controls. Files appear as thumbnails immediately so status, progress, retry, and success actions all live in one place.',
      ...
      dashboardTitle: 'Batch workspace',
      queueTitle: 'Queue and results',
      queueSubtitle: 'Files stay in one grid, switch to converted previews when ready, and keep single-download actions close by.',
      emptyWorkspace: 'Drop HEIC / HEIF files in to populate the workbench with thumbnails and live states.',
    ...
    upload: {
      ...
      hint: 'Up to 80 files, 30 MB per file, and 500 MB per batch.',
      paste: 'Paste support is enabled',
      folder: 'Folder upload is enabled',
      preparing: 'Preparing files',
      ready: 'Ready',
      queueTitle: 'Intake progress',
      queueHint: 'Files are validated first, then added to the workspace before upload or conversion starts.',
      button: 'Choose files',
      errorTitle: 'File check failed'
    },
    settings: {
      modeTitle: 'Processing route',
      modeHint: 'Choose local-first, server-assisted, or automatic routing based on batch size.',
      formatTitle: 'Output format',
      qualityTitle: 'Quality',
      qualityDescription: '82% is the recommended default to keep output size under control and usually within 2x of the source.',
